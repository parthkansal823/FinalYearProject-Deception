...
misses the variant. Others carry no payload at all. The clearest example, and the
...
probing by hand is manual and hostile. A detector that collapses these onto one
score cannot tell the second from the third, and we show in Section 8 that this is
...
the decoy could in principle recognise it. Countering that fully is out of scope —
...
refuses every probe on principle. Section 9 reports what the system does against
...
missed attack is relative to a wrongly diverted user — fixed in advance. It does
