...
- Feature-set **versioning** with a load guard (v4, 18 features).
...
- **Lead result — the causal effect (Contribution 3), because it is the one that
  ...
  probe's effect. Pooled over 99 paired seeds (baited 10,112/10,643 = 0.950 vs
  withheld 1,129/1,244 = 0.908) → effect **+0.029, bootstrap 95% CI [+0.012,
  +0.088]**, odds ratio 2.59, **Fisher exact p = 3.4 × 10⁻¹⁹**. This is the headline; the recall table is
...
- **Recall table, with CIs and honest significance.** Pooled B4 0.943 [0.939,
  ...
  McNemar p=1.9×10⁻⁹⁵** (b=842, c=197 over 11,880 pairs) for the arm difference, and B4
  ahead in **99/99 seeds**. The single-run 0.87→0.90, the v3 20-seed 0.80→0.87
...
### 9. Ablations *(§10.2)*
- **No-bait** = B2 vs B4 in the headline table (recall 0.889 vs 0.943, paired
...
| Fig 10 | **Seed stability** — B4 > B2 in 99/99 paired seeds (paired slope) | `img/seed-stability.svg` |
