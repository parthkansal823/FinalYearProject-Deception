...
Every number below is produced by the multi-seed evaluation driver and its statistical report from one frozen model and is reproduced by the two commands
in the accompanying results document. The report records the digests of the cost table, the bait
library and the meter that produced it, and the figure script refuses to draw from
a report whose digests no longer match the frozen artefacts --- a guard added after a
...
arm & what it does \\
\midrule
\textbf{B0} & no defence; the ceiling on attacker success \\
\textbf{B1} & signature WAF --- regexes for injection, scripting, traversal and command injection, plus scanner user-agents \\
\textbf{B2} & the dual meter, passive: pass or divert, no probe \\
\textbf{B4} & the full system: meter, priced probe, decoy \\
\bottomrule
\end{tabular}
\end{table}


B1, B2 and B4 each ran over \textbf{100 independent seeded traffic draws} of 120 attack
...
password, a user who mistypes URLs), and \textbf{automated-but-harmless clients}: a
...
Proportions carry \textbf{Wilson intervals}, which behave sensibly near zero where the
normal approximation does not --- and every benign rate in this evaluation is near
zero. B2 against B4 uses an \textbf{exact paired McNemar test} on the matched sessions:
...
randomised holdout uses \textbf{Fisher's exact test} with a bootstrap interval on the
...
\textbf{randomised holdout inside the treated arm}: whenever the policy decides to bait,
a deterministic pseudo-random draw over the session id withholds the probe from
about one session in ten. Withheld sessions
...
\midrule
baited (policy) & 10,756 & 10,072 & \textbf{0.936} \\
withheld (holdout) & 1,244 & 1,129 & \textbf{0.908} \\
\bottomrule
\end{tabular}
\end{table}


Effect \textbf{+0.029}, bootstrap 95\% CI \textbf{[+0.012, +0.046]}, odds ratio 1.50, Fisher
exact \textbf{p = 0.00024}. Probing causes diversions that would not otherwise have
...
\subsection{Baselines and recall}
...
\midrule
\textbf{B1} signature WAF & 0.408 [0.399, 0.417] & 0.025 & 0 / 8,000 \\
\textbf{B2} passive & 0.915 [0.910, 0.920] & 0.021 & 4 / 8,000 \\
\textbf{B4} full & \textbf{0.933 [0.929, 0.938]} & 0.024 & \textbf{0 / 8,000} \\
\bottomrule
\end{tabular}
\end{table}
...
concordant, \textbf{499 are caught by B4 alone and 280 by B2 alone}, giving McNemar
\textbf{p < $10^{-4}$}. B4 is ahead in \textbf{78 of 100 seeds}, which is what rules out a lucky
draw.
...
\subsection{Where the probe acts, and where it cannot}
...
\midrule
sqli\_obfuscated & 4,000 & 0.896 & 0.899 & +0.003 & \textbf{0.000} \\
sqli\_stealth & 2,000 & 1.000 & 1.000 & +0.000 & 0.217 \\
idor\_scattered (API) & 2,000 & 0.996 & 0.996 & +0.000 & 0.000 \\
auth\_spray & 2,000 & 1.000 & 1.000 & +0.000 & 0.000 \\
\textbf{idor\_html\_scattered} & 2,000 & \textbf{0.704} & \textbf{0.807} & \textbf{+0.104} & \textbf{0.306} \\
\bottomrule
\end{tabular}
\end{table}
...
McNemar on that subcategory alone of b = 390 against c = 182, \textbf{p < $10^{-4}$}. That is
...
buying. Everywhere else the probe is neutral. \code{sqli\_stealth} bites 22\% of the time
...
\begin{figure}[t]
\centering
\includegraphics[width=0.9\textwidth]{figures/recall-by-category}
\caption{Recall by attack category. The learned system lifts every category over the firewall, but the probe's gain over passive scoring lives entirely in object-reference attacks, which is where the passive detector is undecided.}
\label{fig:bycat}
\end{figure}


One subcategory cannot reach the probe at all, and it is worth stating plainly
because it is half of what B4 still misses. \code{sqli\_obfuscated} has a bite rate of
exactly \textbf{0.000} even though nine in ten of its sessions are shown a probe, and it
accounts for \textbf{405 of B4's 799 misses} (\code{idor\_html\_scattered} accounts for 385,
...
\subsection{Safety, and what it cost the benign side}
...
\textbf{B4 full} & \textbf{0 / 8,000} & 0.0000 [0.0000, 0.0005] \\
\bottomrule
\end{tabular}
\end{table}
...
the way it does. Reporting-integration clients were once diverted \textbf{100\%} of the
...
outcome: \textbf{B1 +3.98, B2 -9.67, B4 -9.84} expected cost per session, against the
\textbf{+15.0} the table implies when nothing is diverted at all. Only the learned arms
...
the \textbf{5th request} in both B2 and B4, with a 90th percentile of 7. Buying
...
\subsection{Outside the simulator}
...
against the full system on an isolated local stack: \textbf{sqlmap 1.10.8}, **ghauri
1.4.3\textbf{ and }wapiti 3.2.3**.

Every cookie-persistent tool is diverted within a handful of requests --- sqlmap on
its 3rd, wapiti on its 13th, both to \code{p \ensuremath{\rightarrow} 1.0} --- and **not one tool bites a probe,
