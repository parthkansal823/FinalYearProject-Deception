...
\caption{The path of a single request. The proxy forwards first and scores afterwards, so a failure in the detection layer cannot affect the user. Every decision, with the features and scores behind it, is written to a hash-chained log.}
...
\subsection{Features and the two-axis meter}
...
Ten \emph{automation} features describe how the client behaves: inter-request timing
...
detection \cite{iliou2019towards,iliou2021detection}. Eight \emph{malice} features describe what
...
Two logistic heads, one per axis, turn the feature vector into an \emph{automation}
score and a \emph{malice} score, each accumulated across the session. Keeping them
...
on choosing \emph{which} probe to deploy, since a scripted scanner and a careful human
take different bait at different rates.


\subsection{The cost policy}
...
\subsection{Bait, and the gate in front of it}

A bait is a token placed on a \emph{channel} of the response, such as an HTML comment,
...
\subsection{The decoy and the Fact Notebook}
...
real once but \emph{staying} consistent --- a fake world that answers the same question
