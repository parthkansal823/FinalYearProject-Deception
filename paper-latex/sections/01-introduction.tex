...
every deployment the deception happens \emph{after} the decision has been made: a
...
a \emph{move the detector can make while it is still unsure} --- a way to manufacture the
...
their intent is hostile. We call these response-side probes \emph{bait}, and the
...
action over a band of belief that is \emph{derived} from the cost of the defender's
errors and the \emph{measured} effectiveness of the probe --- and, we show, a band that
...
commercially. What is new is the decision rule that decides \emph{when} to deceive, and
the fact that its parameters are derived rather than chosen. Concretely:

\begin{enumerate}
 \item \textbf{A priced third action (Section 4).} We show that response-side probing is the
\end{enumerate}
...
\begin{enumerate}
 \item \textbf{A derived evidence weight (Section 5).} The weight a bite carries is a
\end{enumerate}
...
\begin{enumerate}
 \item \textbf{Bait as a randomised treatment (Section 8).} A fixed fraction of sessions
\end{enumerate}
 ...
 and withheld arms is an unbiased estimate of the \emph{causal} effect of baiting,
 not a comparison of two different systems.

\begin{enumerate}
 \item \textbf{A benign corpus built to be hard, and a finding it forced (Section 8).} The
\end{enumerate}

 safety numbers are measured against honest traffic that genuinely looks like an
 attack --- a staff member searching for a colleague named \emph{O'Connell}, a
...
\begin{enumerate}
 \item \textbf{Never worse than its own baseline (Section 9).} Against an adversary that
\end{enumerate}
